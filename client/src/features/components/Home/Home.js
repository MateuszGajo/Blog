...
const Home = (props) => {
    ...
    const handleClick = (value, id) => {
        ...
        const newCategoriesChoose = [...prevCategoriesChoose, { id: id, name: value }];
        ...
        const newData = posts.filter(item => nameCategoriesChoose.every(elem => item.categories.indexOf(elem) > -1));
    ...
    const handleDelete = (id, value) => {
        ...
        const prevCategoriesChoose = categoriesChoose;
        const newCategoriesChoose = prevCategoriesChoose.filter(category => category.id !== id)
        const nameCategoriesChoose = newCategoriesChoose.map(item => item.name);
        const newData = posts.filter(item => nameCategoriesChoose.every(elem => item.categories.indexOf(elem) > -1));
    ...
    useEffect(() => {
        client.query({
            ...
            .then(resp => { setCurrentPosts(resp.data.posts); setPosts(resp.data.posts); setLoading(true) })
            .catch(err => console.log(err))

        client.query({
            query: categoryQuery
        }).then(resp => { setCategories(resp.data.categories); console.log(resp) })
    }, [])
    ...
    return (
        ...
        < Grid container spacing={2} className={classes.card} justify="center" >
            ...
            <Grid item xs={12}>
                {categoriesChoose.length > 0 ? categoriesChoose.map(category => (
                    <Chip
                        key={category.id}
                        className={classes.chip}
                        label={category.name}
